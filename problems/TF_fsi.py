...
common = {"mesh": mesh_file,
          ...
          "rho_s": Constant(10.0E3),  # Solid density (FSI1:1E3, FSI2:10E3, FSI3:1E3) [kg/m3]
          "mu_s": Constant(0.5E6),  # Solid shear modulus or 2nd Lame Coef. (FSI1:0.5E6, FSI2:0.5E6, FSI3:2.0E6) [Pa]
          "nu_s": Constant(0.4),  # Solid Poisson ratio [-]
          "Um": 1.0,  # Max. velocity inlet (FSI1:0.2, FSI2:1.0, FSI3:2.0) [m/s]
...
def initiate(P, v_deg, d_deg, p_deg, dt, theta, dvp_, args, Det_list, refi, mesh_file, mesh_name, **semimp_namespace):
    ...
    if args.extravar == "alfa":
        path = "results/TF_fsi/%(exva)s_%(extype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
