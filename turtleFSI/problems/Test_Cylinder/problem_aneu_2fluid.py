...
def set_problem_parameters(default_variables, **namespace):
    ...
    default_variables.update(dict(
        ...
        rho_f=[1.025E3,1.025E3],    # Fluid density [kg/m3]
        mu_f=[3.5E-3,35E-3],       # Fluid dynamic viscosity [Pa.s]
        ...
        dx_f_id=[1,1001],      # ID of marker in the fluid domain. When reading the mesh, the fuid domain is assigned with a 1.
...
def get_mesh_domain_and_boundaries(mesh_file, fsi_id, rigid_id, outer_wall_id, folder, **namespace):
    ...
    print("The more viscous region extends from y > {}".format(center_y))
    ...
    for submesh_cell in cells(mesh):
        ...
        if idx_cell == 1:
            ...
            mid_y = mid.y()
            if mid_y > center_y:
                domains.array()[i] = 1001 # 1001 is more viscous region
        i += 1


    print("Obtained mesh, domains and boundaries.")
    ff = File("domains/domains.pvd")
    ff << domains
